...
```{r}
getwd()
setwd("C:/Users/helee/OneDrive/Documenten/GitHub/vespa_analyses/Input")
...
-incomplete wil zeggen dat de weergegeves nog niet volledig zijn. Ik
...
volledige dataset - enkel kortste vliegtijden per individu - enkel
...
De datapunten uit Melle hebben duidelijk een groot effect op de
regressielijn. Ze wijkt meer af van de theoretische oranje lijn.
...
Grafieken met outliers telkens voor de volgende datasets - volledige
dataset - enkel kortste vliegtijden per individu - enkel kortste
vliegtijden per wiekpot
...
Telkens voor de modellen Speed \~ Windspeed en Speed \~ Windspeed +
Windspeed²
...
Tailwind significant Perpendicular niet significant Upwind enkel
2egraadsterm licht significant
...
```{r}
plot31<-ggplot(data=subset(Flights_incomplete, !is.na(Observer)), aes(x=Traject25m, y=Speed)) + geom_point(col="slateblue1") + geom_smooth(method="lm", formula =y ~ x, se=TRUE, fullrange=FALSE, level=0.95, color="slateblue4")
               
plot32<-ggplot(data=subset(Flights_incomplete, !is.na(Observer)), aes(x=Traject50m, y=Speed)) + geom_point(col="slateblue1") + geom_smooth(method="lm", formula =y ~ x, se=TRUE, fullrange=FALSE, level=0.95, color="slateblue4")

plot33<-ggplot(data=subset(Flights_incomplete, !is.na(Observer)), aes(x=Traject100m, y=Speed)) + geom_point(col="slateblue1") + geom_smooth(method="lm", formula =y ~ x, se=TRUE, fullrange=FALSE, level=0.95, color="slateblue4")
...
```{r}
plot34<-ggplot(data=subset(Shortind_incomplete, !is.na(Observer)), aes(x=Traject25m, y=Speed)) + geom_point(col="lemonchiffon3") + geom_smooth(method="lm", formula =y ~ x, se=TRUE, fullrange=FALSE, level=0.95, color="lemonchiffon4")
               
plot35<-ggplot(data=subset(Shortind_incomplete, !is.na(Observer)), aes(x=Traject50m, y=Speed)) + geom_point(col="lemonchiffon3") +
geom_smooth(method="lm", formula =y ~ x, se=TRUE, fullrange=FALSE, level=0.95, color="lemonchiffon4")

plot36<-ggplot(data=subset(Shortind_incomplete, !is.na(Observer)), aes(x=Traject100m, y=Speed)) + geom_point(col="lemonchiffon3") + geom_smooth(method="lm", formula =y ~ x, se=TRUE, fullrange=FALSE, level=0.95, color="lemonchiffon4")
...
```{r}
plot37<-ggplot(data=subset(Shortpot_incomplete, !is.na(Observer)), aes(x=Traject25m, y=Speed)) + geom_point(col="grey55") + geom_smooth(method="lm", formula =y ~ x, se=TRUE, fullrange=FALSE, level=0.95, color="black")
               
plot38<-ggplot(data=subset(Shortpot_incomplete, !is.na(Observer)), aes(x=Traject50m, y=Speed)) + geom_point(col="grey55") +
geom_smooth(method="lm", formula =y ~ x, se=TRUE, fullrange=FALSE, level=0.95, color="black")

plot39<-ggplot(data=subset(Shortpot_incomplete, !is.na(Observer)), aes(x=Traject100m, y=Speed)) + geom_point(col="grey55") + geom_smooth(method="lm", formula =y ~ x, se=TRUE, fullrange=FALSE, level=0.95, color="black")

ggarrange(plot37, plot38, plot39 + rremove("x.text"), 
          labels = c("A", "B", "C"),
          ncol = 2, nrow = 2)
```
...
```{r}
model_urb25_all<-lm(Speed ~ Traject25m, na.action=na.exclude, data=Flights_incomplete)
summary(model_urb25_all)

model_urb50_all<-lm(Speed ~ Traject50m, na.action=na.exclude, data=Flights_incomplete)
summary(model_urb50_all)

model_urb100_all<-lm(Speed ~ Traject100m, na.action=na.exclude, data=Flights_incomplete)
...
```{r}
model_urb25_ind<-lm(Speed ~ Traject25m, na.action=na.exclude, data=Shortind_incomplete)
summary(model_urb25_ind)

model_urb50_ind<-lm(Speed ~ Traject50m, na.action=na.exclude, data=Shortind_incomplete)
summary(model_urb50_ind)

model_urb100_ind<-lm(Speed ~ Traject100m, na.action=na.exclude, data=Shortind_incomplete)
...
```{r}
model_urb25_pot<-lm(Speed ~ Traject25m, na.action=na.exclude, data=Shortpot_incomplete)
summary(model_urb25_pot)

model_urb50_pot<-lm(Speed ~ Traject50m, na.action=na.exclude, data=Shortpot_incomplete)
summary(model_urb50_pot)

model_urb100_pot<-lm(Speed ~ Traject100m, na.action=na.exclude, data=Shortpot_incomplete)
summary(model_urb100_pot)
```
...
```{r}
model_1_all<-lm(Flighttime_min ~ Distance + Temperature + Traject25m, na.action=na.exclude, data=Flights_incomplete)
summary(model_1_all)

model_2_all<-lm(Flighttime_min ~ Distance + Temperature + Traject50m, na.action=na.exclude, data=Flights_incomplete)
summary(model_2_all)

model_3_all<-lm(Flighttime_min ~ Distance + Temperature + Traject100m, na.action=na.exclude, data=Flights_incomplete)
summary(model_3_all)
```

## 11) Speed vs Temperatuur + Urbanisation

```{r}
model_4_all<-lm(Speed ~ Temperature + Traject25m, na.action=na.exclude, data=Flights_incomplete)
summary(model_4_all)

model_5_all<-lm(Speed ~ Temperature + Traject50m, na.action=na.exclude, data=Flights_incomplete)
summary(model_5_all)

model_6_all<-lm(Speed ~ Temperature + Traject100m, na.action=na.exclude, data=Flights_incomplete)
summary(model_6_all)
```
