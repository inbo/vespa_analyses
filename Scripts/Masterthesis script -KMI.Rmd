...
```{r}
# zonder interactie
model_temp_urbR<-lmer(ForagingSpeed ~ Temperature_KMI + Urban_EN + (1|Individualcode), na.action=na.omit, data=KMIdata)
...
ggpredict(model_wind1_all, c("WindSpeed_KMI", "Wind_flight")) %>% 
  plot(add.data=T) + scale_x_continuous(name="Wind speed (m/s)") + scale_y_continuous(name="Foraging speed   (m/s)", limits=c(0,7,5)) +  scale_color_manual(values=c('darkorange','seagreen3', 'dodgerblue2'))+
  scale_fill_manual(values=c('darkorange','seagreen3', 'dodgerblue2'), name="fill")+
